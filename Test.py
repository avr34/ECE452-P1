...
class Layer():
    ...
    def MatMul(self, weights: np.ndarray) -> np.ndarray:
        try:
            return np.dot(self.PostActivation, weights.T) + self.Bias
...
class Network():
    ...
    def load_parameters(self, torch_model_path):
        ...
        for i, module in enumerate(linear_modules):
            ...
            if i < len(linear_modules) - 1 and i != 0:
                self.add_layer(out_dim, activation='ReLU')
            else:
                self.add_layer(out_dim, activation='Softmax')
    ...
    def forward(self, input_vector: np.ndarray):
        ...
        try:
            ...
            self.layers[0].PostActivation = input_vector.flatten()
            ...
            for i in range(len(self.layers)):
                ...
                if i + 1 < len(self.layers):
                    ...
                    next_pre = self.layers[i].MatMul(self.extracted_weights[i])
...
def ConvImageToArray(RootFilePath: str) -> tuple[int, np.ndarray]:
    try:
        ...
        with Image.open(RootFilePath) as img:
            ...
            img_array /= 255.0
            label = int(re.search(f'_(\d)_', RootFilePath)[1])
